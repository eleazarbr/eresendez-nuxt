		...
		<div class="columns is-vcentered">
          ...
          <b-icon v-if="post.is_featured" icon="star" class="has-text-warning"></b-icon>
				...
				<div class="text-sm" style="opacity: .8;">
					<p>{{ $t('blog.posted_on', {'date': $moment(post.date).locale($store.getters['lang/locale']).format('LL') }) }}</p>
					<p class="font-bold">{{ post.read_time }}</p>
				</div>

				<!-- Tags -->
				<div class="tags are-small mt-4">
					<b-tag type="is-primary" v-for="(tag, index) in post.tags" :key="index">{{ tag }}</b-tag>
				</div>
			</div>
